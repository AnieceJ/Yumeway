...
function requireGoogleAuth(_req, res, next) {
...
// ==========================================
router.get('/google', requireGoogleAuth, (req, res, next) => {
...
router.get('/google/bind', requireGoogleAuth, (req, res, next) => {
  const { token } = req.query;
  
  if (!token) {
    return res.redirect(`${frontendUrl}/account?error=missing_token`);
  }
...
router.get('/google/callback',
  requireGoogleAuth,
  passport.authenticate('google', { failureRedirect: `${frontendUrl}/user/login?error=oauth_failed` }),
  async (req, res) => {
    try {
      ...
      if (stateToken) {
        try {
          ...
          if (existingGoogleUser && existingGoogleUser.id !== currentUserId) {
            return res.redirect(`${frontendUrl}/user/account/password?error=google_already_linked`);
          }
          ...
          return res.redirect(`${frontendUrl}/user/account/password?success=google_linked`);
        } catch (jwtError) {
          console.error('綁定 JWT 驗證失敗:', jwtError);
          return res.redirect(`${frontendUrl}/user/account/password?error=invalid_token`);
        }
      }
      ...
      if (!fullUser) {
        return res.redirect(`${frontendUrl}/user/login?error=user_not_found`);
      }
      ...
      const frontendUrl = process.env.FRONTEND_URL || `${frontendUrl}`;
      ...
      return res.redirect(`${frontendUrl}/user/login?error=server_error`);
    }
  }
);
